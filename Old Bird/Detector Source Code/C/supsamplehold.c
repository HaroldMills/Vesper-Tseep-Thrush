/*
 * supsamplehold.c: Buffered Polyphase Upsample with Hold
 *
 * Takes buffered input stream, "x1,...,xFactor", upsamples by a number
 * of samples (FACTOR), and outputs "y".  Interpolated samples
 * are held constant, instead of being set equal to zero.
 *
 ...
 *		Size of buffer
 *      Factor
 *      Input phases (must be in increasing order, between 1 and Factor)
 *
 * Author:
 *		Steve Mitchell
 *		Sapphire Computers
...
#define S_FUNCTION_NAME supsamplehold
...
{
	int_T				i, n, j, k, factor, numPhases;
	real_T				*y; 
	InputRealPtrsType	*puPtrs;
	int_T				*pPhases;
	real_T				holdValue;
	...
	for (i=0; i < n; i++)
	{
		...
		{
			if (j == pPhases [k])
				holdValue = *puPtrs [k++][i];
			y [factor * i + j] = holdValue;
		}
	}
}
